...
```
The above tables are written to [files](`r consensusresultsdir`) later.
<br/>
<br/>
...
<br/>
<br/>
And here is the same table for the "invasive-DCIS" comparison. Both tables are also written to [files](`r outputdir`).
...
Result files for all the comparisons with p-values for the two approaches, and with the curation status added, are [here](`r consensusresultsdir`). The tables look like this (top of the DCIS-normal comparison as an example):
...
The diff test, result files saved to [here](`r iClustdifflistdir`).
