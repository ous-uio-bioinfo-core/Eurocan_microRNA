...
voliniatab = list()
voliniatab[["invasive_vs_DCIS"]] = read.table(file=paste(annotdir, "/", volinia_invasive_vs_DCIS_file, sep=""),
                                      sep="\t", check.names=TRUE, 
                                      stringsAsFactors=FALSE, header=TRUE)
voliniatab[["DCIS_vs_normal"]]  = read.table(file=paste(annotdir, "/", volinia_DCIS_vs_normal_file, sep=""),
                                    sep="\t", check.names=TRUE, 
                                    stringsAsFactors=FALSE, header=TRUE)
...
for(n in names(voliniatab))
{
	...
	volinia_invasive_vs_DCIS$fc=log2(volinia_invasive_vs_DCIS$fc)
	names(volinia_invasive_vs_DCIS)[names(volinia_invasive_vs_DCIS)=="fc"] = "log2FC"
	names(volinia_invasive_vs_DCIS)[names(volinia_invasive_vs_DCIS)=="pval"] = "FDR" # this was described as FDR in their paper.
}
# add a MIMATID 
volinia_invasive_vs_DCIS = as.data.frame(append(volinia_invasive_vs_DCIS, 
                  list(MIMATID=mimatmapping[paste("hsa-", volinia_invasive_vs_DCIS$miRNA, sep="")]), 
                  after=1))

volinia_DCIS_vs_normal = as.data.frame(append(volinia_DCIS_vs_normal, 
                  list(MIMATID=mimatmapping[paste("hsa-", volinia_DCIS_vs_normal$miRNA, sep="")]), 
                  after=1))

# use log2 for foldchange in this document.
volinia_invasive_vs_DCIS$fc=log2(volinia_invasive_vs_DCIS$fc)
volinia_DCIS_vs_normal$fc=log2(volinia_DCIS_vs_normal$fc)
names(volinia_invasive_vs_DCIS)[names(volinia_invasive_vs_DCIS)=="fc"] = "log2FC"
names(volinia_DCIS_vs_normal)[names(volinia_DCIS_vs_normal)=="fc"] = "log2FC"
names(volinia_invasive_vs_DCIS)[names(volinia_invasive_vs_DCIS)=="pval"] = "FDR" # this was described as FDR in their paper.
names(volinia_DCIS_vs_normal)[names(volinia_DCIS_vs_normal)=="pval"] = "FDR"
```

```{r results='asis'}
print( xtable(head(volinia_DCIS_vs_normal), caption="Start of table S2 copied from Volinia et al.", digits=3), 
      comment = FALSE,
      type = "html",
...
volinia_overlap = volinia_invasive_vs_DCIS
volinia_overlap = volinia_overlap[!is.na(volinia_overlap$MIMATID),]
rownames(volinia_overlap) = volinia_overlap$MIMATID
volinia_overlap$Direction = ifelse(volinia_overlap$log2FC > 0, "UP", "DOWN")
volinia_overlap = volinia_overlap[, c("Direction","FDR")]

names(volinia_overlap) = paste(names(volinia_overlap), "_vol", sep="")

volinia_overlap = list()

for(contrast %in% )
contrast = "invasive-DCIS"
tab = mergedtables[[contrast]]
a = intersect(  rownames(tab), rownames(volinia_overlap) )
volinia_overlap$Direction_merged = NA
volinia_overlap[a, "Direction_merged"] = ifelse(tab[a, "logFC"] > 0,  "UP", "DOWN")
volinia_overlap$FDR_merged = NA
volinia_overlap[a, "FDR_merged"] = tab[a, "adj.P.Val"]

tab = metatables[[contrast]]
a = intersect(  rownames(tab), rownames(volinia_overlap) )
volinia_overlap$Direction_meta = NA
volinia_overlap[a, "Direction_meta"] = ifelse(tab[a, "logFC"] > 0,  "UP", "DOWN")
volinia_overlap$FDR_meta = NA
volinia_overlap[a, "FDR_merged"] = 0.05
