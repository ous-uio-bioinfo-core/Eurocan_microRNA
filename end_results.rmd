...
- *purple*: no evidence for microRNA expression
- *rejected*: not a microRNA based on structure
- *white*: a microRNA
- *yellow*: maybe a microRNA
...
{
	consensustables[[n]]$curated = curatedtab[ match( (row.names(consensustables[[n]])), curatedtab$MIMAT ), "status"]
...
expressed =	table(curatedtab$status[curatedtab$MIMAT %in% filteredcommonMIMAT])
nonexpressed=	table(curatedtab$status[curatedtab$MIMAT %in% unfilteredcommonMIMAT[!unfilteredcommonMIMAT %in% filteredcommonMIMAT]])
